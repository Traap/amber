...
build Amber's [documentation](https://github.com/Traap/amber/blob/ReqList/doc/amber.pdf)
...
files before they can be automatically consumed.

#### Generated by amber when --language and --browser are used
```bash
../test-output/factory
...
##### PNG files
Any custom program amber invokes can take a screen capture and record it
...
Specific options:
    ...
    -e, --log-environment            List environment
...
option was disabled because Amber writes the commands to the Test Output
...
must be written to reference a program that uses a Web test driver.

##### --language
amber uses English (en) by default.  Your Test Plan, Test Suite, and Test Case
must be written to reference a program that requires internationalization and
localization.

#### Output Factory
##### --log-command
Amber recursively calls Amber.  test-output/commands.log shows each Amber
invocation.
...
use the **--nodryrun** option to cause side effects.  The commands that would
have been executed are echoed to system out.

##### --simulate
The simulate option is used to create a Test Output directory so that you can
design your report.  Each command defined in your Test Case YAML file is echoed
to the Test Output directory Test Case file.
